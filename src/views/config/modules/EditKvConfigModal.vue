...
import { listAllKvConfigType, updateKvConfig } from '@/api/kv-config'
import AFormItem from 'ant-design-vue/es/form/FormItem'
import pick from 'lodash.pick'

export default {
  name: 'EditKvConfigModal',
  ...
  methods: {
    showForm (data) {
      this.$nextTick(() => {
        this.kvConfig = Object.assign({}, data)
        this.fillValuesToEditForm(this.kvConfig)
      })
      this.visible = true
      setTimeout(() => {
    ...
    fillValuesToEditForm (data) {
      ...
      data.selectType = data.type
      const { form } = this
      const formData = pick(data, ['id', 'code', 'value', 'name', 'description', 'inputType', 'selectType'])
      setTimeout(() => {
    ...
    handleSubmit () {
      ...
      this.form.validateFields((errors, values) => {
        if (!errors) {
          updateKvConfig(this.kvConfig).then(res => {
            if (res && res.success) {
              this.visible = false
