...
import { queryClientAuthoritiesByCode, updateClientDetail } from '@/api/client'
...
export default {
  ...
  methods: {
    showForm (data) {
      this.visible = true
      this.$nextTick(() => {
        this.clientDetail = Object.assign({}, data)
        this.fillValuesToEditForm(this.clientDetail)
      })
      setTimeout(() => {
    ...
    fillValuesToEditForm (data) {
      const { form } = this
      this.accessTokenValidity = data.accessTokenValidity
      this.refreshTokenValidity = data.refreshTokenValidity
      this.resourceIdAddList = Object.assign([], data.resourceIdList)
      this.scopeList = Object.assign([], data.scopeList)
      const formData = pick(data, ['clientId', 'clientSecret', 'resourceIdList', 'scopeList',
        'authorizedGrantTypeList', 'authorityList', 'autoapprove', 'webServerRedirectUriList',
        'refreshTokenValidity', 'accessTokenValidity', 'additionalInformation'])
      setTimeout(() => {
    ...
    handleSubmit () {
      ...
      this.form.validateFields((errors, values) => {
        if (!errors) {
          updateClientDetail(this.clientDetail).then(res => {
            if (res && res.success) {
              this.$message.info('修改Client信息成功')
              this.visible = false
              this.$emit('ok', this.kvConfig)
            } else {
              this.$message.error('修改Client信息成功:', res.msg)
            }
          })
        }
